...
async function run() {
  try {
    ...
    const jobApplicationsCollections = client.db("jobPortal").collection("jobApplications");

    // Auth Related APIs
    app.post("/jwt", async (req, res) => {
      const user = req.body;
      const token = jwt.sign(user, process.env.JWT_SECRET, { expiresIn: '1h'});
      // To generate a token, in terminal run:
      // 'node' and then run: 'require('crypto').randomBytes(64).toString('hex')'
    ...
    app.patch("/jobApplications/:id", async (req, res)=>{
      ...
      const result = await jobApplicationsCollections.updateOne(filter, updateDoc);
      res.send(result);
    })
